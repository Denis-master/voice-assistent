        ...
        if best_statement is None or best_statement.confidence < self.google_treshold:
            ...
            for st in statements:
                if self.check_in_string(st.text, ('вперёд', 'иди', 'шагай')):
                    ...
                    command_recognized = True
                elif self.check_in_string(st.text, ('Включи свет', 'включи свет')):
                    client.connect("192.168.2.190", 1883)
                    answer = "Включаю свет"
                    client.publish("relay",1)
                    client.disconnect() 
                    command_recognized = True
                elif self.check_in_string(st.text, ('Выключи свет', 'выключи свет')):
                    client.connect("192.168.2.190", 1883)
                    answer = "Выключаю свет"
                    client.publish("relay",0)
                    client.disconnect() 
                    command_recognized = True
                elif self.check_in_string(st.text, ('какая погода сегодня', 'скажи погоду')):                
                    fame = datetime.date.today()
                
                    wethd = open("/home/denis/bot/"+str(fame) + ".txt","r")
    
                    answer="Сегодня температура "+str(wethd.readlines()[-3])+" градусов по цельсию. "
                    
                    wethd.close()
                    wethd = open("/home/denis/bot/"+str(fame) + ".txt","r")
                    
                    
                    answer=answer+"Влажность "+str(wethd.readlines()[-2])+" процентов. "
                    wethd.close()
                    wethd = open("/home/denis/bot/"+str(fame) + ".txt","r")
                    
                    answer=answer+"Давление "+str(wethd.readlines()[-1])+" миллиметров ртутного столба. "
